...
# Read keeping raw line endings
with open(path, "r", encoding="utf-8", newline="") as f:
    content = f.read()

# Let's list the errors we found and check if they exist exactly in `content`
errors = [
    {
...
print("Checking matches with raw line endings:")
all_ok = True
for idx, err in enumerate(errors):
    orig = err["original"]
    count = content.count(orig)
    if count == 0:
        print(f"ERROR: No match found for '{repr(orig)}'")
        all_ok = False
    elif count > 1:
        print(f"WARNING: Multiple ({count}) matches found for '{repr(orig)}'")
    else:
        print(f"OK: Match found for '{repr(orig)}'")

if all_ok:
    print("All errors matched exactly!")
else:
    print("Some errors did not match. Please verify line endings/characters.")
